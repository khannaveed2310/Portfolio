...
export function InteractiveTerminal() {
        ...
        output = (
          <div className="p-2.5 rounded-lg bg-emerald-950/40 border border-emerald-500/30 text-xs space-y-1 text-emerald-300">
            ...
            <p className="text-slate-300 text-[11px]">
              Ready to ship production code. Redirecting to transmission channel in 2 seconds...
            </p>
          </div>
        );
        setTimeout(() => {
  ...
  return (
    <div className="w-full rounded-2xl overflow-hidden border border-cyan-500/20 bg-slate-950/85 backdrop-blur-xl shadow-2xl shadow-cyan-950/30 font-mono text-left">
      {/* Terminal Titlebar */}
      <div className="bg-slate-900/90 px-4 py-2.5 flex items-center justify-between border-b border-slate-800/80">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 ml-2 text-xs text-slate-400">
            ...
            <span className="text-slate-300 font-medium">naveed@architect:~ (interactive-cli)</span>
          </div>
        </div>
      ...
      <div ref={outputContainerRef} className="p-4 h-64 overflow-y-auto space-y-3 text-xs text-slate-300 scrollbar-thin">
        {history.map((item) => (
